/* main.c -- Dariusburst SP .so loader
 *
...
SDL_Window *SDL_CreateWindow_hook(const char *title, int x, int y, int w, int h, Uint32 flags) {
	return SDL_CreateWindow("dariusburst", 0, 0, w, h, flags);
}
...
void *(*new)(size_t sz);
void (*BatchOptimizer_Clear)(void *this);
...
float *GetFloatArrayElements(void *env, void *obj) {
	return NULL;
}
...
void *audio_thd(void *argv) {
	void (*adx2Drive)(void *env, int obj, char *media_path, uint8_t opensl) =  (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_adx2Drive");
	adx2Drive(&fake_env, 0, "ux0:data/dariusburst", 0);
	sceKernelExitDeleteThread(0);
	return NULL;
}

static unsigned int size_draw_v = 276;
static unsigned char draw_v[] __attribute__((aligned(16))) = {
...
static unsigned int size_draw_f = 256;
static unsigned char draw_f[] __attribute__((aligned(16))) = {
...
void *real_main(void *argv) {
	float *pos = (float *)malloc(sizeof(float) * 8);
	float *texcoord = (float *)malloc(sizeof(float) * 8);
	pos[0] = pos[2] = pos[3] = pos[7] = -1.0f;
	pos[1] = pos[4] = pos[5] = pos[6] = 1.0f;
	texcoord[0] = texcoord[2] = texcoord[3] = texcoord[7] = 0.0f;
	texcoord[1] = texcoord[4] = texcoord[5] = texcoord[6] = 1.0f;
	...
	char fname[256];
	sprintf(data_path, "ux0:data/dariusburst");
	
	printf("Loading libdariusburst\n");
	sprintf(fname, "%s/libdariusburst.so", data_path);
	...
	*(uintptr_t *)(fake_env + 0x2F4) = (uintptr_t)GetFloatArrayElements;
	...
	*(uintptr_t *)(fake_env + 0x374) = (uintptr_t)GetStringUTFRegion;
	...
	int (*create)(void *env, int obj, char *res_path, int unk, char *save_path, char *lang, char *unk2, uint8_t big_width) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_create");
	int *(*onDrawFrame)(void *env, int obj, int unk, int *array) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onDrawFrame");
	int (*onResume)(void *env, int obj) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onResume");
	void (*onSurfaceChanged)(void *env, int obj, int unk, int w, int h) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onSurfaceChanged");

	pthread_t t;
	pthread_attr_t attr;
	pthread_attr_init(&attr);
	pthread_attr_setstacksize(&attr, 0x400000);
	pthread_create(&t, &attr, audio_thd, NULL);
	
	void (*onPressDpadUp)(void *env, int obj, uint8_t pressed) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onPressDpadUp");
	void (*onPressDpadDown)(void *env, int obj, uint8_t pressed) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onPressDpadDown");
	void (*onPressDpadLeft)(void *env, int obj, uint8_t pressed) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onPressDpadLeft");
	void (*onPressDpadRight)(void *env, int obj, uint8_t pressed) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onPressDpadRight");
	void (*onPressDecideButton)(void *env, int obj, uint8_t pressed) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onPressDecideButton");
	void (*onPressXButton)(void *env, int obj, uint8_t pressed) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onPressXButton");
	void (*onPressYButton)(void *env, int obj, uint8_t pressed) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onPressYButton");
	void (*onPressR1Button)(void *env, int obj, uint8_t pressed) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onPressR1Button");
	void (*onPressL1Button)(void *env, int obj, uint8_t pressed) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onPressL1Button");
	void (*onLeftStickMove)(void *env, int obj, float x, float y) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onLeftStickMove");
	void (*onBackKeyPressed)(void *env, int obj) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onBackKeyPressed");
	void (*onTouch)(void *env, int obj, int action, int x, int y) = (void *)so_symbol(&main_mod, "Java_jp_co_taito_am_dariusBurstSP_JNILib_onTouch");
	
	int lang;
	sceAppUtilSystemParamGetInt(SCE_SYSTEM_PARAM_ID_LANG, &lang);
	if (lang == SCE_SYSTEM_PARAM_LANG_JAPANESE)
		create(&fake_env, 0, "ux0:data/dariusburst", 1, "ux0:data/dariusburst", "ja", "ja", 1);
	else
		create(&fake_env, 0, "ux0:data/dariusburst", 1, "ux0:data/dariusburst", "en", "ja", 1);
	onResume(&fake_env, 0);
	onSurfaceChanged(&fake_env, 0, 0, SCREEN_W * 2, SCREEN_H * 2);
	...
	vglShaderGxpBinary(1, &vs, draw_v, size_draw_v);

	GLuint fs = glCreateShader(GL_FRAGMENT_SHADER);
	vglShaderGxpBinary(1, &fs, draw_f, size_draw_f);

	GLuint fb_prog = glCreateProgram();
	glAttachShader(fb_prog, vs);
	glAttachShader(fb_prog, fs);
	glBindAttribLocation(fb_prog, 0, "inPos");
	glBindAttribLocation(fb_prog, 1, "inTex");
	glUniform1i(glGetUniformLocation(fb_prog, "tex"), 0);
	glLinkProgram(fb_prog);
	...
	for (;;) {
		...
		if ((pad.buttons & SCE_CTRL_UP) != (oldpad & SCE_CTRL_UP)) {
			onPressDpadUp(&fake_env, 0, ((pad.buttons & SCE_CTRL_UP) == SCE_CTRL_UP) ? 1 : 0);
		}
		if ((pad.buttons & SCE_CTRL_DOWN) != (oldpad & SCE_CTRL_DOWN)) {
			onPressDpadDown(&fake_env, 0, ((pad.buttons & SCE_CTRL_DOWN) == SCE_CTRL_DOWN) ? 1 : 0);
		}
		if ((pad.buttons & SCE_CTRL_LEFT) != (oldpad & SCE_CTRL_LEFT)) {
			onPressDpadLeft(&fake_env, 0, ((pad.buttons & SCE_CTRL_LEFT) == SCE_CTRL_LEFT) ? 1 : 0);
		}
		if ((pad.buttons & SCE_CTRL_RIGHT) != (oldpad & SCE_CTRL_RIGHT)) {
			onPressDpadRight(&fake_env, 0, ((pad.buttons & SCE_CTRL_RIGHT) == SCE_CTRL_RIGHT) ? 1 : 0);
		}
		if ((pad.buttons & SCE_CTRL_CROSS) != (oldpad & SCE_CTRL_CROSS)) {
			onPressDecideButton(&fake_env, 0, ((pad.buttons & SCE_CTRL_CROSS) == SCE_CTRL_CROSS) ? 1 : 0);
		}
		if ((pad.buttons & SCE_CTRL_SQUARE) != (oldpad & SCE_CTRL_SQUARE)) {
			onPressXButton(&fake_env, 0, ((pad.buttons & SCE_CTRL_SQUARE) == SCE_CTRL_SQUARE) ? 1 : 0);
		}
		if ((pad.buttons & SCE_CTRL_TRIANGLE) != (oldpad & SCE_CTRL_TRIANGLE)) {
			onPressYButton(&fake_env, 0, ((pad.buttons & SCE_CTRL_TRIANGLE) == SCE_CTRL_TRIANGLE) ? 1 : 0);
		}
		if ((pad.buttons & SCE_CTRL_LTRIGGER) != (oldpad & SCE_CTRL_LTRIGGER)) {
			onPressL1Button(&fake_env, 0, ((pad.buttons & SCE_CTRL_LTRIGGER) == SCE_CTRL_LTRIGGER) ? 1 : 0);
		}
		...
		static int lastX[6] = { -1, -1, -1, -1, -1, -1 };
		static int lastY[6] = { -1, -1, -1, -1, -1, -1 };
		for (int i = 0; i < 6; i++) {
			if (i < touch.reportNum) {
				...
				if (lastX[i] == -1 || lastY[i] == -1) {
					onTouch(fake_env, NULL, 1, x, y);
				} else if (lastX[i] != x || lastY[i] != y) {
					onTouch(fake_env, NULL, 2, x, y);
				}
				lastX[i] = x;
				lastY[i] = y;
			} else {
				if (lastX[i] != -1 || lastY[i] != -1) {
					onTouch(fake_env, NULL, 3, lastX[i], lastY[i]);
				}
				lastX[i] = -1;
				lastY[i] = -1;
			}
		...
		glBindFramebuffer(GL_FRAMEBUFFER, fb);
		onDrawFrame(&fake_env, 0, 0, NULL);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		glBindTexture(GL_TEXTURE_2D, fb_tex);
		glViewport(0, 0, 960, 544);
		glClear(GL_COLOR_BUFFER_BIT);
		glUseProgram(fb_prog);
		glEnableVertexAttribArray(0);
        glEnableVertexAttribArray(1);
        glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 0, &pos[0]);
        glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 0, &texcoord[0]);
